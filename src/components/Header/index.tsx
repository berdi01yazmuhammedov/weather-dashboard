...
export const Header = ({
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://api.weatherapi.com/v1/search.json?key=de85bd798d4e407bbf613122252608&q=${searchValue}`
        );
        const data = await res.json();
        setSuggestions(data.map((item: Suggestions) => item.name));
      } catch (error) {
        console.error(error);
      }
    }, 400);
  ...
  const onClickSuggestion = (city: string) => {
    setQuery(city);
    setSearchValue("");
    setSuggestions([]);
    setIsError(false);
      ...
      <div className={styles.header}>
        <form onSubmit={onSubmitForm}>
          ...
          <div className={styles.search}>
            ...
            <button type="submit">
              {searchValue ? (
                <img onClick={onClickX} src={cancelSVG} alt="Cancel icon" />
              ) : (
        ...
        <div className={styles.profile}>
          <img width={100} src={profilePhoto} alt="Profile" />
        </div>
      </div>
      {isError ? (
        <SearchError lastFailedQuery={lastFailedQuery} />
      ) : (
        <h2 className={styles.cityName}>{query}</h2>
      )}
    </>
  );
};
